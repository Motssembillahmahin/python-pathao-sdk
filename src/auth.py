...
class AuthManager:
    def __init__(
        ...
        password: str,
        base_url: str,
        store_type: str = "memory",  # memory, file, custom
    ):
    ...
    def _store_token(self, token_response: Dict):
        """Store token with expiration calculation"""
...
class PathaoAuth(httpx.Auth):
    ...
    def sync_auth_flow(self, request):
        token = asyncio.run(self.auth_manager.get_access_token())
        request.headers["Authorization"] = f"Bearer {token}"
        response = yield request

        if response.status_code == 401:
            asyncio.run(self.auth_manager._refresh_or_authenticate())
            token = asyncio.run(self.auth_manager.get_access_token())
            request.headers["Authorization"] = f"Bearer {token}"
            yield request
